...
class DualPhaseGenerator:
    ...
    def generate(self, 
                 ...
                 bg_grid_cols: Optional[int] = None) -> Dict[str, Any]:
        ...
        # Filename - Simplified format: MovieAds-MMDD-ID
        if date_str is None:
        ...
        filename = f"MovieAds-{date_str}-{video_id:02d}.mp4"
    ...
    def generate_phase_a(self, avatar_path: str) -> Tuple[CompositeVideoClip, float]:
        ...
        if self.assets.get('png_applist'):
            ...
            applist_clip = self._create_floating_panel(path, 'left', phase_duration, top_limit=searchbox_bottom_y)
            
        textlist_clip = None
        if self.assets.get('png_textlist'):
            path = random.choice(self.assets['png_textlist'])
            textlist_clip = self._create_floating_panel(path, 'right', phase_duration, top_limit=searchbox_bottom_y)
            
        # Ad Text
        adtext_clip = None
        if self.assets.get('png_adtext'):
            path = random.choice(self.assets['png_adtext'])
            adtext_clip = self._create_pulsing_adtext(path, phase_duration)
    ...
    def generate_phase_b(self, movie_path: str) -> Tuple[CompositeVideoClip, float]:
        ...
        # Overlays
        logger.info("   🎬 Generating dynamic overlay sequence...")
        overlay_events = self._generate_overlay_events(phase_duration, phase='B')
    ...
    def _create_pulsing_adtext(self, path: str, duration: float) -> ImageClip:
    ...
    def _generate_overlay_events(self, total_duration: float, phase: str = 'B') -> List[VideoFileClip]:
        events = []
        current_time = 0.25
